...
export const HubSpotCookieBanner: React.FC = () => {
  ...
  const handleShowBanner = () => {
    ...
    if (!success) {
      if (isInIframe) {
        alert('Vous êtes dans l\'environnement de prévisualisation.\n' + 'La bannière de cookies HubSpot sera disponible en production.');
      } else {
        alert('La bannière de cookies HubSpot n\'est pas disponible.\n' + 'Vérifiez la configuration dans votre compte HubSpot.');
      }
    }
  };
  const resetCookies = () => {
    const cookiesToRemove = ['__hs_cookie_cat_pref', 'hubspotutk', '__hstc', '__hssc', '__hssrc'];
  ...
  if (isProduction) {
    return <button type="button" onClick={handleShowBanner} className="text-sm text-muted-foreground hover:text-foreground transition-colors duration-200" title="Paramètres des cookies">
