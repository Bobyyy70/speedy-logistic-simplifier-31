...
const Header = () => {
  ...
  const navigationItems = [
    ...
    { name: "Technologie", url: "/technology", icon: Laptop },
    ...
    <header className={cn(
      ...
      <div className="container flex h-20 items-center justify-between py-0 my-[8px]">
        ...
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger>
            <Button variant="ghost" size="sm" className="h-10 w-10 hover:border-speedelog-500 hover:text-speedelog-500 dark:hover:text-speedelog-400 md:hidden" aria-label="Menu">
              <Menu className="h-5 w-5" />
            </Button>
          </SheetTrigger>
          <SheetContent side="right" className="w-[80%] sm:w-[350px] bg-white dark:bg-slate-900 backdrop-blur-md">
            <SheetHeader>
                ...
                <SheetClose key={item.name} asChild>
                  <NavLink 
                    ...
                    className={({isActive}) => cn(
                      "text-base font-medium transition-colors hover:text-speedelog-500 dark:hover:text-speedelog-400 px-1 py-2", 
                      isActive ? "text-speedelog-500 dark:text-speedelog-400 font-semibold" : "text-foreground"
                    )}
                  >
                    {item.name}
                  </NavLink>
                ...
                <SheetClose>
                  <NavLink to="/contact" className="block">
                    <Button 
                      variant="solid" 
                      size="default" 
                      className="bg-speedelog-500 hover:bg-speedelog-600 text-white"
                    >
                      Obtenir un devis
                    </Button>
                  </NavLink>
      ...
      {isMobile && (
        <NavBar items={navigationItems.slice(0, 5)} className="z-50" />
      )}
    </header>
  );
};
