...
export function LogisticsFeatureSection({ backgroundVariant = 'white' }: { backgroundVariant?: 'white' | 'site' }) {
  ...
  const globalShippingPoints = [
    // Destinations nord/sud équilibrées
    {
    ...
    // Destinations intercontinentales sélectionnées
    {
      start: { lat: 48.8566, lng: 2.3522, label: "France" },
      ...
      <div className="container mx-auto px-4 mt-12">
        ...
        <div className="mobile-grid-3 gap-6">
          {checkItems.map((item, index) => (
            <motion.div
              key={index}
              className="bg-white align-center border border-gray-100 rounded-lg p-6 shadow-sm hover:shadow-md transition-all duration-300"
              custom={index}
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true }}
