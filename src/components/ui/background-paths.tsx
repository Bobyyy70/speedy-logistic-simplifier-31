...
function FloatingPaths({ position, opacity = 0.15 }: { position: number; opacity?: number }) {
  const paths = Array.from({ length: 36 }, (_, i) => ({
    ...
    width: 0.3 + i * 0.02, // Lignes plus fines pour un effet plus subtil
  }));
      ...
      >
        ...
        {paths.map((path) => (
          <motion.path
            ...
            strokeOpacity={(0.05 + path.id * 0.005) * opacity} // Opacité très réduite
            initial={{ pathLength: 0.3, opacity: 0.2 }}
            animate={{
              pathLength: 1,
              opacity: [0.1, 0.2, 0.1], // Valeurs d'opacité très réduites
              pathOffset: [0, 1, 0],
            }}
            transition={{
              duration: 20 + Math.random() * 10, // Animation plus lente
...
export function BackgroundPaths({
  className,
  children,
  opacity = 0.15, // Opacité par défaut très réduite
