...
export function FAQ() {
  return (
    <section className="bg-gradient-to-r from-blue-100 via-white to-green-200 dark:from-slate-900 dark:via-slate-950 dark:to-green-900 py-12 md:py-20">
      <div className="section-container">
        <h2 className="section-title">Questions fréquentes</h2>
        <p className="section-subtitle">
          Trouvez les réponses à vos questions sur nos services de logistique e-commerce
        </p>
          ...
          <Accordion type="single" collapsible>
            {faqs.map((faq, index) => (
              <AccordionItem 
                key={index} 
                value={`item-${index}`}
                className="border-orange-100/20 dark:border-orange-900/20"
              >
                <AccordionTrigger className="hover:text-orange-600 dark:hover:text-orange-400 transition-colors">
                  {faq.question}
                </AccordionTrigger>
